...
Locator: getByText(/dashboard|analytics|admin/i).first()
Expected: visible
Timeout: 20000ms
...
Call log:
  ...
  - waiting for getByText(/dashboard|analytics|admin/i).first()
    - waiting for" http://localhost:3000/login?email=admin%40truehire.com&password=admin123" navigation to finish...
    - navigated to "http://localhost:3000/login?email=admin%40truehire.com&password=admin123"
...
> 27 |     await expect(this.page.getByText(landingSignals[role]).first()).toBeVisible({ timeout: 20_000 });
  ...
  40 | 
  41 |     const form = this.page.locator('form').filter({ has: button }).first();
  42 |     if (await form.count()) {
  43 |       await form.evaluate((element) => element.requestSubmit());
  44 |       return;
  45 |     }
  46 | 
