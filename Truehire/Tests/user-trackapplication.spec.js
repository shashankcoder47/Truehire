...
test.describe("Track Applications Flow", () => {
  const user = {
    id: 201,
    name: "Rakshitha R",
  ...
  const jobDetailsById = {
    ...
    502: {
      ...
      salary_max: "110000",
      salary_currency: "USD",
      description: "Ship product UI with React and Next.js.",
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: /track applications/i }),
    ).toBeVisible({ timeout: 15000 });
  });

  test("Click Track Applications and validate Applications page", async ({ page }) => {
    ...
    await expect(modal.getByText(/job details/i)).toBeVisible();
    ...
    await expect(page.getByText(/job details/i)).toHaveCount(0);

    await withdrawBtn.first().click();
    await expect(page.getByText("Data Scientist", { exact: true })).toHaveCount(0);

    await backBtn.click();
    await expect(page).toHaveURL(/\/overview(?:\?|$)|\/user-dashboard(?:\?|$)/i);
  });
});
